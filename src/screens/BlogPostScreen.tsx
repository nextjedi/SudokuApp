...
export const BlogPostScreen: React.FC<BlogPostScreenProps> = ({ postId, onBack }) => {
    ...
    return (
      <SafeAreaView style={styles.container}>
        <View style={styles.notFound}>
          ...
          <Text style={styles.notFoundText}>The blog post you're looking for doesn't exist.</Text>
  ...
  const renderContent = () => {
    ...
    lines.forEach((line) => {
      if (line.startsWith('# ')) {
        elements.push(<Text key={key++} style={styles.contentH1}>{line.substring(2)}</Text>);
      } else if (line.startsWith('## ')) {
        elements.push(<Text key={key++} style={styles.contentH2}>{line.substring(3)}</Text>);
      } else if (line.startsWith('### ')) {
        elements.push(<Text key={key++} style={styles.contentH3}>{line.substring(4)}</Text>);
      } else if (line.startsWith('- ')) {
        elements.push(
        ...
        elements.push(<Text key={key++} style={styles.contentP}>{line}</Text>);
      }
    });
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.postCategoryBadge}>
          <Text style={styles.badgeEmoji}>{getCategoryEmoji(post.category)}</Text>
